...
<main class="main-content position-relative border-radius-lg">
    <div class="container-fluid mt-6rem">
        <form action="{{ (isset($action) ? route('admin.pmtupdate') : route('admin.pmtstore') ) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="hd_id" value="{{ (isset($result) ? $result->id : '') }}">

            <div class="row nav-fixed">
                <div class="col-6">
                    <Strong class="">เพิ่มข้อมูลโปรโมชั่น</Strong>
                </div>
                <div class="col-6 text-end">
                    <button type="submit" class="btn btn-success my-0 me-3">บันทึก</button>
                    <a href="{{ route('admin.pmt') }}" class="btn btn-warning my-0">ยกเลิก</a>
                </div>
            </div>
            <div class="row">
                <div class="col-9">
